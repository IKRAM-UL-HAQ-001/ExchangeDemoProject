...
@section('content')
                            ...
                            <table id="user" class="table align-items-center mb-0 table-striped table-hover px-2">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-uppercase text-secondary font-weight-bolder text-dark ps-2">Exchange
                                            Name</th>
                                ...
                                <tbody>
                                    @foreach ($userRecords as $user)
                                        <tr data-user-id="{{ $user->id ?? 'N/A' }}"
                                            data-exchange-id="{{ $user->exchange->id ?? 'N/A' }}">
                                            <td>{{ $user->name ?? 'N/A' }}</td>
                                            <td>{{ $user->exchange->name ?? 'N/A' }}</td>
                                            <td>{{ $user->created_at ?? 'N/A' }}</td>
                                            <td class="text-center">
                                                <form action="{{ route('assistant.user.status') }}" method="POST"
                    ...
                    <div class="modal-body">
                        <form id="addUserForm">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control border px-3" id="name"
                    ...
                    <div class="modal-body">
                        <form id="editUserForm">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control border px-3" id="editName"
                                    placeholder="Enter User Name" required>
                            </div>
                            <div class="mb-3">
    ...
    <script>
        ...
        function editUser(button) {
            ...
            const exchangeName = row.find('td:nth-child(2)').text();
            const type = row.find('td:nth-child(2)').text().toLowerCase(); // Assuming it's in lowercase
            const password = ""; // Can't fetch password directly for security reasons

            // Fill modal inputs
            $('#editUserId').val(userId);
            $('#editName').val(userName.trim());
            $('#editExchange').val(exchangeName.trim());
            $('#editType').val(type.trim());
